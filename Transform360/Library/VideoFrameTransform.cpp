...
static float
intersectSphereOffset(float x, float y, float z, float ox, float oy, float oz) {
  float loc;
  float odot;
  float root;
...
static Mat calculateKernel(float sigma) {
  ...
  float sum = 0;
  float sigmaComponent = std::abs(sigma) < kEpsilon ? 0 : 0.5 / (sigma * sigma);
...
static double
angularDistance(double yaw1, double pitch1, double yaw2, double pitch2) {
  return acos(
      sin(pitch1) * sin(pitch2) + cos(pitch1) * cos(pitch2) * cos(yaw1 - yaw2));
}
...
static double getEffectiveRatio(double angularDist, double offset, double fov) {
  ...
  if (angularDist - kEpsilon > fov / 2) {
    if (angularDist + fov / 2 > M_PI) {
      double edge1 =
          samplingArc(offset, (2 * M_PI - angularDist - fov / 2) * 2) / 2;
      ...
      majorAxisScaling = (samplingArc(offset, 2 * angularDist + fov) -
                          samplingArc(offset, 2 * angularDist - fov)) /
          2 / fov;
    }
  } else {
    majorAxisScaling = (samplingArc(offset, 2 * angularDist + fov) +
                        samplingArc(offset, fov - 2 * angularDist)) /
        2 / fov;
  }

  double distToCoVertex = angularDistance(angularDist, 0.5 * fov, 0.0, 0.0);
  double minorAxisScaling =
      samplingArc(offset, distToCoVertex * 2) / (distToCoVertex * 2);
...
VideoFrameTransform::VideoFrameTransform(FrameTransformContext* ctx) {
...
void VideoFrameTransform::generateKernelAndFilteringConfig(
  ...
  float sigmaX = min(0.5 * inputWidth, sigmaY / (cos(angle) + kEpsilon));

  // Build a basic 1D kernel along X direction
  Mat kernelX = calculateKernel(sigmaX);

  int numHorizontalSegments =
      ctx_.adjust_kernel ? ctx_.num_horizontal_segments : 1;
  ...
  for (int i = 0; i < numHorizontalSegments && i * segmentWidth < inputWidth;
    ...
    if (ctx_.adjust_kernel) {
      ...
      double dist = angularDistance(yaw, pitch, avgYaw, avgPitch);
...
void VideoFrameTransform::calcualteFilteringConfig(
  ...
  switch (ctx_.output_layout) {
    ...
    case LAYOUT_N: {
      printf("Invalid layout type for plane %d.\n", transformMatPlaneIndex);
      return;
    }
  }

  float sigmaY = 0.5f *
      std::min(ctx_.max_kernel_half_height,
               std::max(
...
bool VideoFrameTransform::generateMapForPlane(
  ...
  try {
    assert(
        ...
        outputHeight > 0 && ctx_.width_scale_factor > 0 &&
        ctx_.height_scale_factor > 0 && ctx_.kernel_height_scale_factor > 0 &&
    ...
    // purpose
    int scaledOutputWidth = (int)(ctx_.width_scale_factor * outputWidth + 0.5);
    ...
    printf(
        "Could not generate map for plane %d. Error: %s\n",
        transformMatPlaneIndex,
        ex.what());
    return false;
  }
...
void VideoFrameTransform::runFiltering(
  ...
  for (int i = 0; i < segmentFilteringConfigs_[transformMatPlaneIndex].size();
       ++i) {
    ...
    switch (ctx_.interpolation_alg) {
      ...
      case LANCZOS4: {
        ...
        if (ctx_.enable_low_pass_filter) {
          ...
          tempMat =
              filterPlane(inputMat, transformMatPlaneIndex, imagePlaneIndex);
        } else {
          tempMat = inputMat;
        }
  ...
  try {
    ...
    switch (ctx_.output_layout) {
      ...
      case LAYOUT_BARREL_SPLIT:
        ...
        //
        {
          if (3.0f * x <= 2.0f) {
            vFace = (int)(y * 2);
            yaw = ((3.0f / 2.0f * x - 0.5f) * ctx_.expand_coef - vFace + 1.0f) *
                M_PI;
    ...
    switch (ctx_.output_layout) {
      ...
      case LAYOUT_EAC_32: {
        ...
        if (std::abs(ctx_.fixed_cube_offcenter_x) > kEpsilon ||
          ...
          if (ctx_.is_horizontal_offset) {
            ...
            dist = intersectSphereOffset(
                ...
                0,
                qz,
                ctx_.fixed_cube_offcenter_x,
                0,
                ctx_.fixed_cube_offcenter_z);
            if (dist > 0.0f) {
        ...
        tx = qx * (c1 * c3 + s1 * s2 * s3) - qy * (c3 * s1 * s2 - c1 * s3) +
            qz * (c2 * s1);
        ty = qx * (c2 * s3) - qy * (c2 * c3) + qz * (-s2);
        tz = qx * (c1 * s2 * s3 - c3 * s1) - qy * (c1 * c3 * s2 + s1 * s3) +
            qz * (c1 * c2);
      ...
      case LAYOUT_FLAT_FIXED: {
        ...
        *outY =
            ((y - 0.5f) * ctx_.fixed_vfov - ctx_.fixed_pitch) / 180.0f + 0.5f;
...
bool VideoFrameTransform::transformFramePlane(
  ...
  try {
    Mat inputMat(
        inputHeight, inputWidth, CV_8U, inputArray, inputWidthWithPadding);

    Mat outputMat(
        outputHeight, outputWidth, CV_8U, outputArray, outputWidthWithPadding);
